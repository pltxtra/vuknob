                   ...
                   unsigned char key, unsigned char rvelocity)
{
    ...
    if (instance->monophonic) {  /* monophonic mode */
	    ...
	    DEBUG_MESSAGE(DB_NOTE, "      * monophonic mode - note off\n");
	    ...
	    DEBUG_MESSAGE(DB_NOTE, "      * polyphonic mode - note off\n");
        if (HEXTER_INSTANCE_SUSTAINED(instance)) {
...
void
dx7_lfo_update(hexter_instance_t *instance, unsigned int sample_count)
{
    unsigned int sample;

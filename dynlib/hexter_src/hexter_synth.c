                         ...
                         unsigned char rvelocity)
{
    ...
    DEBUG_MESSAGE(DB_NOTE, "instance_note_off\n");
                                ...
                                int param, signed int value)
{
    ...
    /* update edit buffer */
    instance->current_patch_buffer[((5 - opnum) * 21) + param] = value;
 ...
 */
void
hexter_instance_select_program(hexter_instance_t *instance, unsigned int bank,
 ...
 */
void
hexter_synth_render_voices(unsigned int samples_done,
                           unsigned int sample_count, int do_control_update)
{
...
/*
 * hexter_synth_render_instance_voices
 */
void
hexter_synth_render_instance_voices(
	hexter_instance_t *instance,
	...
	unsigned int sample_count, int do_control_update)
{
    ...
    for (i = 0; i < hexter_synth.global_polyphony; i++) {
        ...
        if (_PLAYING(voice) && voice->instance == instance) {
            if (voice->mods_serial != voice->instance->mods_serial) {
                dx7_voice_update_mod_depths(voice->instance, voice);
                voice->mods_serial = voice->instance->mods_serial;
            }

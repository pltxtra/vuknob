...
tar extends Object
	...
	private String		archiveName;
		...
		/// This variable holds the base directory where the files that go into an archive are located.
	 ...
	 */
	public static void
	tar_function( String argv[] ) throws com.ice.tar.InvalidHeaderException, java.io.IOException
		{
		tar app = new tar();
	 ...
	 */
	public void
	instanceMain( String argv[] ) throws com.ice.tar.InvalidHeaderException, java.io.IOException
		{
		...
		if ( archive == null )
			...
			for ( ; argIdx < argv.length ; ++argIdx )
				{
					String base = "";
					String fname = argv[ argIdx ];
					if(archiveBaseDir != null)
						base = archiveBaseDir + "/";

					//	try {
					Log.v("TAR", "Archiving new file " + fname);
					File f = new File( base + fname );
					Log.v("TAR", "  !! ST1 " + fname);
				
					TarEntry entry = new TarEntry(base, f);
					Log.v("TAR", "  !! ST2 " + fname);
					...
					Log.v("TAR", "  !! ST3 " + argv[ argIdx ]);

					archive.writeEntry( entry, true );
					Log.v("TAR", "  !! ARCHIVED " + argv[ argIdx ]);
//					}
//				catch ( IOException ex )
//				{
//					ex.printStackTrace( System.err );

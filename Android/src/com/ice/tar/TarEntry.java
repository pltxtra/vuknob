...
implements	Cloneable
	...
	public
	TarEntry( String base, File file )
		throws InvalidHeaderException
		{
		this.initialize();
		baseDir = base;
	...
	/**
	 * Set this entry's file.
	 *
	 * @param file This entry's new file.
	 */
	public void
	setFile(File file)
		{
	...
	getFileTarHeader( TarHeader hdr, File file )
		...
		// strip the baseDir name from the path.
		name = name.substring( baseDir.length(), name.length() );
		Log.v("TarEntry", "    ADDING FILE : " + name );
		...
		for ( int i = 0 ; i < list.length ; ++i )
			{
				TarEntry nt = 
				new TarEntry
					( baseDir, new File( this.file, list[i] ) );
				result[i] = nt;
			}

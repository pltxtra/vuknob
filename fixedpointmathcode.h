...
#ifndef HAS_AUTOGEN_NAT_LOG_LUT
...
static void __bump_me();

static void __fixed_point_math_make_compiler_warnings_go_away(fp8p8_t *a, fp24p8_t *b, fp16p16_t *c, fp8p24_t *d)
{
	(*a) = _sqrtufp8p8(1);
	(*b) = _sqrtufp24p8(1);
	(*c) = _sqrtufp16p16(1);
...
static void __bump_me() {
	...
	fp16p16_t c;
	fp8p24_t d;
	__fixed_point_math_make_compiler_warnings_go_away(&a, &b, &c, &d);
}
#endif
